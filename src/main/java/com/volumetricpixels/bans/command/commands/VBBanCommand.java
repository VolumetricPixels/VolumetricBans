...
public class VBBanCommand extends VBCommand {
	...
	public void execute(final CommandSource source, final Command cmd,
		...
		try {
			...
			final boolean global = globalStr.equals("yes")
					|| Boolean.parseBoolean(globalStr);
			...
			if (reason == null || reason.equals("")) {
				if (!global) {
					if (temp) {
						...
						reason = "You are temporarily banned from this server until "
								+ Ban.df.format(date);
					} else {
			...
			if (temp) {
				sHandler.getBans().add(
						new Ban(plugin, target, reason.toString(), source
								.getName(), time));
			} else {
				sHandler.getBans().add(
						new Ban(plugin, target, global, reason.toString(),
								source.getName()));
			}
		} catch (final ArrayIndexOutOfBoundsException e) {
			throw new CommandException(
					"Invalid syntax, /vb ban <player> [-t time] [-g] [ban reason]\n"
