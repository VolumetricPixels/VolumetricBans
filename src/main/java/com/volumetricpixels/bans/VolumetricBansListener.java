...
//      }
		...
		plugin.getEngine().getScheduler()
				.scheduleAsyncTask(plugin, cache, true);
	}
	...
	public void onPlayerJoin(final PlayerLoginEvent event) {
		...
		if (pm.isBanned(name)) {
			...
			plugin.getLogger().info(
					"Prevented banned player '" + name + "' from logging in!");
		} else if (plugin.isOnlineMode()) {
//          if (plugin.isPremium()) {
			if (cache.recent.containsKey(name)) {
				if (cache.recent.get(name) && player != null) {
					player.kick("You are globally banned from VolumetricBans servers, see volumetricbans.net!");
					plugin.getLogger().info(
							"Kicked globally permabanned player " + name + "!");
				}
			} else {
